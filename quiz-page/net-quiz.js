...
async function fetchQuestions() {
  try {
    const response = await fetch("http://localhost:8080/api/net-questions"); // API URL
    if (!response.ok) {
...
function checkAnswer(selectedOption, button) {
  ...
  // after 1 second move to the next question after selecting an answer
...
function nextQuestion() {
  if (currentQuestionIndex < questions.length - 1) {
    ...
    window.location.href = `/result-page/result.html`; // Redirect to results page
  }
}
